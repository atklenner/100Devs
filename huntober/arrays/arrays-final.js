...
const Game = () => {
  ...
  function checkWin(arr1, arr2) {
    ...
    setIsWin(flatArr1.every((letter, index) => flatArr2[index] === letter));
  }
  ...
  function moveBlankLeft(curGrid, blankChar) {
    ...
    let movedValue = curGrid[index - 1];
    curGrid[index] = movedValue;
  ...
  function moveBlankUp(curGrid, blankGrid) {
    curGrid.forEach((row, index) => {
      if (row.includes(blankGrid) && index >= 1) {
        let column = row.indexOf(blankGrid);
        let valueToBeSwapped = curGrid[index - 1][column];
        curGrid[index][column] = valueToBeSwapped;
        curGrid[index - 1][column] = blankGrid;
      }
    });
    checkWin(gridWithBlank, curGrid);
    return curGrid;
  ...
  function moveBlankDown(curGrid, blankGrid) {
    let moved = false;
    curGrid.forEach((row, index) => {
      if (row.includes(blankGrid) && index < curGrid.length - 1 && !moved) {
        let column = row.indexOf(blankGrid);
        let valueToBeSwapped = curGrid[index + 1][column];
        curGrid[index][column] = valueToBeSwapped;
        curGrid[index + 1][column] = blankGrid;
